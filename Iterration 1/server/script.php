...
if (isset($_REQUEST['action']) && $_REQUEST['action']=='Update'){
  ...
  $annee = $_REQUEST['annee_de_sortie'];
  $image = $_REQUEST['url_image'];
  $trailer = $_REQUEST['url_trailer'];
  $type = $_REQUEST['type'];
  $ok = updateFilms($titre , $real , $annee , $image , $trailer , $type);
  exit(); 
}
// termine le script (ce qui est en dessous ne s'exécutera pas)
...
//   }
//   exit(); // termine le script (ce qui est en dessous ne s'exécutera pas)s
// }
...
if ( isset($_REQUEST['action'] ) && $_REQUEST['action'] == 'getFilm' ){
    $filmentier = getFilm();
    echo json_encode($filmentier);
    exit(); // termine le script (ce qui est en dessous ne s'exécutera pas)
}
